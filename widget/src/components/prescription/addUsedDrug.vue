        ...
        <van-checkbox :name="item.itemId" v-for="(item,index) in drugList.list" :key="index" @click="selectChange(item)" >
          <div class="drugInfo">
            ...
            >用法：
              ...
              、每次{{item.eachQuan}}{{item.jldw}}、{{item.ddufName}}、用药{{item.days}}天</li>
            <li
              class="usage" v-else
            >用法：
              <span v-if="item.dduName">{{item.dduName}}</span>
              <span v-if="item.djdName">{{item.djdName}}</span>
              、每次{{item.eachQuan}}{{item.jldw}}</li>
            <li class="priceNum">
              <u>价格：¥{{item.price}}</u>
              <u>x{{item.quan}}</u>
            </li>
            <li class="explanation" v-if="item.memo">说明：
            {{item.memo}}</li>
          </div>
        </van-checkbox>
      </van-checkbox-group>
  ...
  export default {
    ...
    methods: {
      ...
      getDetail() {
          ...
          findDocCommonPrescriptionDetail(param).then(res => {
            console.log(res, "常用处方详情");
            ...
            if (drugSelect == element.itemId) {
              ...
              if (isNaN(idx)) {
                ...
                for (let i = 0; i < arr[idx].length; i++) {
                  ...
                  console.log(info.drugName,element.drugName)
