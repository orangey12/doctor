<template>
  <div class="thisCheckList">
    ...
    <div class="infoDiv">
      ...
      <div v-if="info.pacsListMVO && info.pacsListMVO.length > 0">
        <div class="checkList font-size-md" v-for="(item,index) in info.pacsListMVO" :key="index" @click="openDetail(item)">
          <li class="display-div">
            <u class="font-blod">检查编号：{{ item.ppiCode}}</u>
            <u class="font-login-color-red" v-if="item.status == '1' || item.status == '2' || item.status == '3'">{{item.statusName}}</u>
          ...
          <li class="display-div " v-for="(pascItem,pascIndex) in item.pacsListDVO" :key="pascIndex">
            <u>{{pascIndex + 1}}.{{pascItem.itemName}}-{{pascItem.positionName}}</u>
