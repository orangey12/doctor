        ...
        <van-pull-refresh v-model="isLoading" @refresh="onRefresh">
          ...
          >
            <div class="prePoolList">
              <div class="card" @click="openDetail(item)" v-for="(item,index) in preList" :key="index">
...
import {queryProPrescriptionPoolListPage, queryProPrescriptionPoolReceivingListPage} from "../../api/cf";
...
export default {
  ...
  methods: {
    ...
    getList() {
      if (this.active == 0) {
        queryProPrescriptionPoolListPage(this.listQuery).then(res => {
          this.setData(res);
        })
      } else {
        ...
        queryProPrescriptionPoolReceivingListPage(param).then(res => {
          this.setData(res);
        })
      }
      ...
      if(this.active == '1'){
        for (let i = 0; i < data.length; i++) {
          ...
          var time = date.DateDifference(
            element.receivingOrderTime.replace(/-/g, "/"),
            element.currentTime.replace(/-/g, "/"),
            element.receivingOrderEffectiveTime
          );
          if (time.days > 0) {
            element.remaining =
